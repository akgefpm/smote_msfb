...
def train_model(X_scaled, y, model, resample_model, path, version, splits):
    ...
    for train_index, test_index in splits: # kf.split(X_scaled, y):
        ...
        pd.DataFrame(X_test).to_csv(path + "/X_test_"+str(fold_num)+".csv")
        pd.DataFrame(X_train).to_csv(path + "/X_train_"+str(fold_num)+".csv")                       
        pd.DataFrame(y_test).to_csv(path + "/y_test_"+str(fold_num)+".csv")
        pd.DataFrame(y_train).to_csv(path + "/y_train_"+str(fold_num)+".csv")
        ...
        if resample_model == "smotten_sf_mfs_p":
            ...
            X_train_file_path = path + "/X_train_upd_"+str(resample_model)+"_"+str(fold_num)+"_resampled_data.csv"
            y_train_file_path = path + "/y_train_upd_"+str(resample_model)+"_"+str(fold_num)+"_resampled_data.csv"
            ...
            ## Case 2 - if the resampled file does not exists in the folder. Do the resampling and store the file
                ...
                pd.DataFrame(X_train_upd).to_csv(path + "/X_train_upd_"+str(resample_model)+"_"+str(fold_num)+"_resampled_data.csv")
                pd.DataFrame(y_train_upd).to_csv(path + "/y_train_upd_"+str(resample_model)+"_"+str(fold_num)+"_resampled_data.csv")
        ...
        pd.DataFrame(y_prob).to_csv(path + "/y_prob_"+str(version)+"_"+str(fold_num)+".csv")
    ...
    # Save to file
    roc_data_all_folds_m.to_csv(path + "/roc_data_manual_threshold_" + str(version) + ".csv", index=False)
    roc_data_all_folds.to_csv(path + "/roc_data_" + str(version) + ".csv", index=False)
    metrics_df.to_csv(path + "/metrics_"+version+".csv")
    cv_metrics_df.to_csv(path + "/Cross_Validation_metrics_"+version+".csv")
    resampling_time_df.to_csv(path + "/Resampling_time_metrics_"+version+".csv")
        
    return 1 #avg_precision, avg_recall, avg_roc_auc, metrics_df, avg_fpr, avg_tpr
